...
public class TeamsMenu extends PageablePlayerInventory {
    ...
    private static final GameProfile BACK, SAVE, OPEN;

    static {
        BACK = new GameProfile(UUID.randomUUID(), null);
        BACK.getProperties().put("textures", new Property("textures",
    ...
    public TeamsMenu(Player viewer) {
        ...
        if (hp != null && hp.getAbstractArena() != null) {
            ...
            for (int i = 0; i < hp.getAbstractArena().getConfiguration().getIslands(); i++) {
                Teams team = Teams.values()[i];
                Team t = hp.getAbstractArena().getTeams().get(team);
                boolean hasCreated = hp.getAbstractArena().getTeams().get(team).isTotallyConfigured();
                items.add(new ItemBuilder(hasCreated ? Material.WOOL : Material.STAINED_GLASS).durability(team.getColor().getData()).name((hasCreated ? "§a✔" : "§c✖" )+ " §7Configuração do time: " + team.getDisplayName()).lore(
                        "",
                        " §c⚑ Localizações do time: ",
                        "",
                        "  §a⤷ §7Local de spawn: " + (t.getSpawnLocation() != null ? "§a§l✔" : "§c§l✖"),
                        "  §a⤷ §7Local da cama: " + (t.getBedLocation() != null ? "§a§l✔" : "§c§l✖"),
                        "  §a⤷ §7Local da do NPC de Melhorias: " + (t.getUpgradeLocation() != null ? "§a§l✔" : "§c§l✖"),
                        "  §a⤷ §7Local da do NPC de Loja: " + (t.getShopLocation() != null ? "§a§l✔" : "§c§l✖"),
                        "",
                        " §b⦿ Bordas e geradores: ",
                        "  §a⤷ §7Borda da ilha criada: " + (t.getBorder() != null ? "§a§l✔" : "§c§l✖"),
                        "  §a⤷ §7Quantidade de geradores criados: §b" + t.getTeamGenerators().size(),
                        "",
    ...
    public void click(Inventory inv, ItemStack item, int slot) {
        ...
        if (item.getType() == Material.SKULL_ITEM && item.getItemMeta().getDisplayName().startsWith("§9Página anterior")) {
            ...
            new MainPainel(getPlayer(), HylexPlayer.get(getPlayer()).getAbstractArena());
            ...
            HylexPlayer.get(player).setAuxiler(team);
            ...
            player.getInventory().setItem(4, new ItemBuilder(Material.STAINED_GLASS_PANE).durability(item.getDurability()).name("§aConfigurando o time: " + team.getDisplayName() + " §7(Clique para alterar)").build());
